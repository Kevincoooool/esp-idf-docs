...
# Windows 搭建 ESP-IDF 开发环境

> **版本说明**：本页以 ESP-IDF **5.3.3** 安装为例（图文步骤仍适用）。  
> **视频系列**：[A01](./文档总目录与视频教程大纲.md#三系列-a--零基础入门必拍) · **总索引**：[文档总目录与视频教程大纲](./文档总目录与视频教程大纲.md)  
> 酷世 DIY **Korvo S3** 和 **ESP32P4-C5** 例程推荐使用 **5.5.x**（如 5.5.3），安装时在安装器中选择对应版本并勾选 **ESP32-P4** 目标（P4 板必需）。  
> 例程编译流程见 [例程开发流程与公共组件](./例程开发流程与公共组件.md)。  
> 建议阅读顺序：[ESP-IDF 框架整体构成](./通用-ESP-IDF框架构成.md) → [工程结构（对比 Keil）](./通用-ESP-IDF工程结构.md)。
...
如果无法下载也可以进百度云链接（**仅 ESP-IDF 安装包**，非例程资料）：

[https://pan.baidu.com/s/11jwTFJbcOsV2K_IlkBriJQ?pwd=qm34](https://pan.baidu.com/s/11jwTFJbcOsV2K_IlkBriJQ?pwd=qm34) 提取码: qm34

5.3.x～5.5.x 均可安装；**Korvo / P4C5 例程请选 5.5.3**。小智等项目 5.4+ 也支持。

这个链接是乐鑫官方的下载网，可以在这里下载离线安装包，选择所需版本（如 5.5.3 或 5.3.3）
...
P4C5 开发板需先执行 `idf.py set-target esp32p4`；Korvo / SP V4 用 `esp32s3`。
